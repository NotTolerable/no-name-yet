...
| Dependency graph | A curated catalog and deterministic offline graph utility load versioned JSON, validate configuration, resolve prerequisites, and topologically order controls. The graph is not called by the verification pipeline and does not assess readiness. | Dependency-aware assessment and remediation consume the graph without allowing it to create evidence or authorize claims. |
...
The current Technical State Map is an in-memory collection of extracted facts. Its labels are not yet mapped to canonical catalog IDs, and its data remains separate from the implemented dependency graph.
...
The implemented graph foundation defines `ControlDefinition`, `ControlDependency`, `DependencyType`, `ControlCatalog`, and `ControlDependencySet`. Catalog identifiers and the validated semantic version are product knowledge. `REQUIRED` edges may block readiness in future assessment; `SUPPORTING` edges provide maturity context and do not automatically block readiness. No `ControlAssessment` exists yet.
...
The graph is human-curated, deterministic, versioned, validated, and acyclic. The current `1.0.0` JSON catalog and dependency set are loaded by `backend/core/dependency_graph.py`. Unknown endpoints, duplicate or ambiguous edges, self-dependencies, version mismatches, invalid identifiers, and cycles fail closed. Queries return direct or transitive prerequisites by dependency type, and topological ordering places prerequisites first with stable control-ID tie-breaking.

This utility is not invoked by `trust_packet.py`, policy, answers, remediation, persistence, API routes, workflows, or the frontend. It neither reads facts nor calculates readiness. Assessment integration and recording the graph version in results belong to PR 3.
...
Current tests cover Pydantic models, deterministic extraction, matching, policy, answer and deficit generation, graph loading and validation, deterministic graph queries, the trust-packet pipeline, API contracts, and an in-memory persistence adapter. They do not cover dependency-aware readiness or remediation and do not verify a live Supabase project or any target workflow, AI, upload, export, or authentication capability.
...
2. Add versioned control catalog and dependency graph. *(Foundation implemented; readiness integration remains separate.)*
